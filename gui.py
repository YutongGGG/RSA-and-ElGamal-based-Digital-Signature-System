...
class MainWindow(Windows, Screen):
    ...
    def update_label(self, value):
        ...
        self.ids.file_status_label.text = self.ids.file_status_label.text + '[font=OpenSans-Regular]File is not opened.[/font]' \
            if not Windows.input_file_name \
        ...
        self.ids.opened_status_label.text = "[font=OpenSans-Light]Key status: [/font][font=OpenSans-Regular]Keys are not set! \n" \
                                            "Please, set up keys in [font=OpenSans-Bold]key settings[/font] menu[/font]" \
            if not Windows.are_keys_set \
...
class MainWindow2(Windows, Screen):
    """
    Main window 2, that includes file signing functions
    """
    text_input2 = ObjectProperty(None)
    _create_signature2 = BooleanProperty(True)
    _check_signature2 = BooleanProperty(False)
    ...
    def on_enter2(self):
        Clock.schedule_once(self.update_label2)
    ...
    def update_label2(self, value):
        ...
        :return: none
        """
        self.ids.opened_status_label2.text = "[font=OpenSans-Light]Key status: [/font][font=OpenSans-Regular]Keys are not set! \n" \
                                             "Please, set up keys in [font=OpenSans-Bold]key settings[/font] menu[/font]" \
            if not Windows.are_keys_set \
            else "[font=OpenSans-Light]Key status: [/font][font=OpenSans-Regular]Keys are set successfully.[/font]"

    @property
    def create_signature2(self):
        return self._create_signature2

    @create_signature2.setter
    def create_signature2(self, value):
        self._create_signature2 = value
        self._check_signature2 = not value

    @property
    def check_signature2(self):
        return self._check_signature2

    @check_signature2.setter
    def check_signature2(self, value):
        self._check_signature2 = value
        self._create_signature2 = not value

    def sign_message(self, _message, signature_k, signature_r, signature_s):
        """
        Creating fiele digital signature
        :param signature_s:
        :param signature_r:
        :param signature_k:
        :param _message:
        ...
        r, s, k = Elgamal(p, a).signing(message)
        signature_k.text = str(k)
        signature_r.text = str(r)
        signature_s.text = str(s)
        return signature_r.text, signature_s.text, _message.text

    def is_signature_valid2(self, signature_check_r, signature_check_s, _check_message):
        """
        Handling 'Check signature' button. Checking if file signature is valid
        :param _check_message:
        :param signature_check_s:
        :param signature_check_r:
        :param _signature: signature text field
        :param _public_key: public key text field
        :param _n_key: n key text field
        :return: None
        """
        signature_check_r = int(signature_check_r.text)
        signature_check_s = int(signature_check_s.text)
        _check_message = _check_message.text

        keys = loadKeys()
        p, a = keys['p'], keys['a']
        v1, v2, valid = Elgamal(p, a).verifying(signature_check_r, signature_check_s, _check_message)
        self.ids.check_file_hash.text = str(v1)
        self.ids.check_file_hash_recovered.text = str(v2)
        if v1 == v2:
            self.ids.is_valid_signature_label.text = '[font=OpenSans-Bold]Signature is valid[/font]'
        else:
            self.ids.is_valid_signature_label.text = '[font=OpenSans-Bold]Signature is invalid[/font]'
...
class SettingKeys2(Windows, Screen):
    """
    Class of handling settings windows
    """

    def save_keys2(self, p_value, a_value, x_value, y_value):
        """
        Saving key button. It handles all fields in setting and filling it
        automately. If fields are filled not correctly, it raises error
        :param p_value: p field
        :param a_value: a field
        :param x_value: x (private key) field
        :param y_value: y (public key) field
        :return: None
        """
        Windows.are_keys_set = True
        content = ErrorDialog(message="Saved successful.", close=self.dismiss_popup)
    ...
    @staticmethod
    def generate_random_key2(key, value):
        """
        Generates p or q key
        :param key: p or q field
        :param value: string 'p' or 'q'
        :return: int value of key
        """
        key_result = loadKeys(filename='keys.txt')
        key.text = str(key_result[value])
        return int(key.text)

    def generate_private_key2(self, text_area, key):
        """
        Generating public or private key depends on field to fill
        :param text_area: 'e' of 'd' field in settings
        :param key: string 'e' (public key) or 'd' (private key)
        ...
        try:
            ...
            text_area.text = str(private[key])
            return int(text_area.text)
        except KeyError:
            content = ErrorDialog(message="x error", close=self.dismiss_popup)
            self._popup = Popup(title="Key generating", content=content,
                                size_hint=(0.9, 0.9))
            self._popup.open()

    def generate_public_key2(self, text_area, key):
        """
        Generating public or private key depends on field to fill
        :param text_area: 'e' of 'd' field in settings
        :param key: string 'e' (public key) or 'd' (private key)
        ...
        p, a = keys['p'], keys['a']
        try:
            public = Elgamal(p, a).pubicKey()
            text_area.text = str(public[key])
            return int(text_area.text)
        except KeyError:
            content = ErrorDialog(message="y error", close=self.dismiss_popup)
            self._popup = Popup(title="Key generating", content=content,
                                size_hint=(0.9, 0.9))
            self._popup.open()
...
class LoadDialog(FloatLayout):
    ...
    def get_default_path(self):
        self.path = os.path.expanduser("/Users/yugii/Desktop")  # Get desktop path.
        return self.path
...
presentation = Builder.load_file("rsasignature2.kv")
...
Factory.register('MainWindow2', cls=MainWindow)
Factory.register('SettingKeys2', cls=SettingKeys)
